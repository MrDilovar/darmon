...
@section('content')
  <div class="container py-4">
    <h3 class="mb-3">Редактировать продукт</h3>
    <div class="row">
      <div class="col-lg-6">
        <form class="formsValidate" action="{{ route('admin.product.update', $product->id) }}" method="post"  enctype="multipart/form-data" novalidate>
          ...
          <div class="form-group">
            ...
            <select name="category" class="custom-select" id="inputCategory" required>
              ...
              @foreach($categories as $category)
                <option value="{{ $category->id }}" {{ (old('category') ? old('category') : $product->categories->first()->id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
              @endforeach
            </select>
          ...
          <div class="form-group">
            ...
            <div class="mb-3">
              <img style="max-height: 250px;" src="{{ '/' . $PATH_TO_PRODUCT_IMG . $product->image }}" alt="...">
            </div>
